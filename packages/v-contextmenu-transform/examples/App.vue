<template>
  <div id="app">
    <header>
      ...
      <p>
        ...
        <iframe
          ...
          width="170px"
          height="20px"
          src="https://ghbtns.com/github-btn.html?user=heynext&repo=v-contextmenu&type=star&count=true"
          frameborder="0"
        ></iframe>
      </p>
    </header>
    ...
    <div class="usage">
      <h3>用法</h3>
      <a
        :class="['usage-link', theme]"
        href="https://github.com/heynext/v-contextmenu/blob/master/README.md"
      >
        安装 &amp 使用
        <i class="v-contextmenu-iconfont usage-icon"></i>
      </a>
    </div>
    ...
    <main class="showground">
      <div :class="['example-title', theme]">「{{ exampleTitle }}」示例</div>

      <component :is="currentExample" :theme="theme"></component>

      <p>
        查看源码：
        <a :href="currentExampleSourceUrl" target="_blank">{{ currentExampleFileName }}</a>
      </p>
    </main>
  </div>
</template>
...
import SubmenuTransform from './SubMenuTransfrom.vue'
...
const EXAMPLE_LIST = [{
  ...
  id: 'SubmenuTransform',
  label: '子菜单(使用 transform 兼容 scroll)',
  source: '',
}, {
  id: 'Disabled',
  label: '禁用',
